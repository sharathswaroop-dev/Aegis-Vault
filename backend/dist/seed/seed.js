...
async function seed() {
    try {
        ...
        await Lot_1.Lot.deleteMany({});
        await Warehouse_1.Warehouse.deleteMany({});
        await Distributor_1.Distributor.deleteMany({});
        await Retailer_1.Retailer.deleteMany({});
        // Create docs without links first to get their ObjectIds
        const createdUsers = await User_1.User.create(farmersData.map(f => ({ ...f, linkedPartners: [] })));
        const createdWarehouses = await Warehouse_1.Warehouse.create(warehousesData.map(w => ({ ...w, linkedFarmers: [], linkedDistributors: [], status: "active" })));
        const createdDistributors = await Distributor_1.Distributor.create(distributorsData.map(d => ({ ...d, linkedWarehouses: [], linkedRetailers: [], status: "active", activeShipments: Math.floor(Math.random() * 10) })));
        const idMap = {};
        createdUsers.forEach(u => idMap[u.customId] = u._id);
        createdWarehouses.forEach(w => idMap[w.customId] = w._id);
        createdDistributors.forEach(d => idMap[d.customId] = d._id);
        // Create retailers with correct dist id
        const createdRetailers = await Retailer_1.Retailer.create(retailersData.map(r => ({
            ...r,
            linkedDistributor: idMap[r.linkedDistributorId],
            status: "active"
        })));
        createdRetailers.forEach(r => idMap[r.customId] = r._id);
        // Update users with correct links
        for (const f of farmersData) {
            if (f.linkedPartnerIds.length > 0) {
                await User_1.User.updateOne({ customId: f.customId }, { $set: { linkedPartners: f.linkedPartnerIds.map(id => idMap[id]) } });
            }
        }
        // Update warehouses
        for (const w of warehousesData) {
            await Warehouse_1.Warehouse.updateOne({ customId: w.customId }, {
                $set: {
        ...
        for (const d of distributorsData) {
            await Distributor_1.Distributor.updateOne({ customId: d.customId }, {
                $set: {
        ...
        await Lot_1.Lot.create(lots);
        console.log("✅ Seed data created!");
        console.log(`Users: ${await User_1.User.countDocuments()}`);
        console.log(`Lots: ${await Lot_1.Lot.countDocuments()}`);
        console.log(`Warehouses: ${await Warehouse_1.Warehouse.countDocuments()}`);
        console.log(`Distributors: ${await Distributor_1.Distributor.countDocuments()}`);
        console.log(`Retailers: ${await Retailer_1.Retailer.countDocuments()}`);
        process.exit(0);
    }
    catch (error) {

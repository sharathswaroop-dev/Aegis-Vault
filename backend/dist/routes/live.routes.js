...
router.get("/weather", async (_req, res) => {
    const data = await getCached(realtime_poller_1.FF_CHANNELS.WEATHER);
    if (data)
        return res.json({ success: true, data, source: "redis" });
    // Fallback: direct fetch if Redis is cold
    try {
        const locRes = await axios_1.default.get("https://ipapi.co/json", { timeout: 6000 });
        const { latitude: lat, longitude: lng, city, region, country, ip } = locRes.data;
        const wRes = await axios_1.default.get(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current_weather=true&timezone=auto&forecast_days=1`, { timeout: 6000 });
        ...
        res.status(503).json({ error: "Weather unavailable", message: err.message });
    }
});
// GET /api/live/location
router.get("/location", async (_req, res) => {
    const data = await getCached(realtime_poller_1.FF_CHANNELS.LOCATION);
    if (data)
        return res.json({ success: true, data, source: "redis" });
    try {
...
router.get("/prices", async (_req, res) => {
    const data = await getCached(realtime_poller_1.FF_CHANNELS.PRICES);
    if (data)
        return res.json({ success: true, data, source: "redis" });
...
router.get("/orders", async (_req, res) => {
    const data = await getCached(realtime_poller_1.FF_CHANNELS.ORDERS);
    if (data)
        return res.json({ success: true, data, source: "redis" });
...
router.get("/alerts", async (_req, res) => {
    const data = await getCached(realtime_poller_1.FF_CHANNELS.ALERTS);
    if (data)
        return res.json({ success: true, data, source: "redis" });
...
router.get("/hub/:id", async (req, res) => {
    ...
    const t0 = Date.now();
    try {
        await axios_1.default.get("https://worldtimeapi.org/api/ip", { timeout: 5000 });
    ...
    // Get ISP/org info
    const locationData = await getCached(realtime_poller_1.FF_CHANNELS.LOCATION);
    ...
    const det = (offset, min, max) => min + ((seed + offset) % (max - min + 1));
    ...
    const status = latencyMs > 800 ? "critical" : latencyMs > 400 ? "degraded" : "healthy";
    res.json({
        success: true,
        data: {
            hubId,

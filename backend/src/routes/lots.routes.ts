...
import { redisPub, CHANNELS } from "../lib/redis";
...
router.get("/", optionalAuth, async (req: any, res: Response) => {
  try {
    ...
    if (role === "farmer" && userId) filter.farmerId = userId;
    else if (role === "warehouse" && userId) filter.warehouseId = userId;
    else if (role === "distributor" && userId) filter.distributorId = userId;
    else if (role === "retailer" && userId) filter.retailerId = userId;
    ...
    const processedLots = lots.map(lot => {
      ...
      if (expiryDiffDays <= 3) freshnessStatus = "urgent";
      else if (expiryDiffDays <= 7) freshnessStatus = "at-risk";
...
router.post("/:id/action", authenticate, async (req: any, res: Response) => {
  try {
    ...
    if (action === "harvested") lot.status = "harvested";
    else if (action === "arrived-warehouse") lot.status = "in-warehouse";
    else if (action === "picked-up") lot.status = "in-transit";
    else if (action === "delivered") lot.status = "delivered";
    
    await lot.save();
    
    // Publish update to Redis
    redisPub.publish(CHANNELS.LOT_UPDATES, JSON.stringify({ lotId: lot._id, status: lot.status, action }));
...
router.post("/:id/qr", authenticate, async (req: any, res: Response) => {
  try {
    const lot = await Lot.findById(req.params.id);
    if (!lot) return res.status(404).json({ error: "Lot not found" });
...
router.post("/scan", authenticate, async (req: any, res: Response) => {
  try {
    const { qrCode, location, action } = req.body;
    const lot = await Lot.findOne({ qrCode });
    if (!lot) return res.status(404).json({ error: "Invalid QR Code" });
